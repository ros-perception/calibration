...
if __name__ == '__main__':
    ...
    while(lineind < len(xmllines)):
        ...
        if looking_for_calibration_pos or looking_for_origin:
            ...
            if looking_for_origin and not looking_for_calibration_pos:
                print "  Need to only do a transform update"
            ...
            elif (not looking_for_origin) and looking_for_calibration_pos:
                print "  Need to only do a calibration position update"

                # Do the math
                print "    Applying a joint offset of %f" % joint_offsets[joint_name]
                origin_q = rpy_to_quat(origin_rpy)
                offset_q = transformations.quaternion_about_axis(joint_offsets[joint_name], axis_xyz)
                total_q  = transformations.quaternion_multiply(origin_q, offset_q)
                next_origin_rpy = quat_to_rpy(total_q)
                ...
                newline = "\t<origin rpy=\"%.10f %.10f %.10f\" xyz=\"%.10f %.10f %.10f\"/>"% (next_origin_rpy + tuple(origin_xyz)) + oldlineending
                print "    transform old line:", joint_lines[origin_ind].strip()
                print "    transform new line:", newline.strip()
                joint_lines[origin_ind] = newline
            else:
                print "  Need to update both"

                # Do the math
                print "    Applying a joint offset of %f" % joint_offsets[joint_name]
                origin_q = rpy_to_quat(rpy_trans[0:3])
                offset_q = transformations.quaternion_about_axis(joint_offsets[joint_name], axis_xyz)
                total_q  = transformations.quaternion_multiply(origin_q, offset_q)
                next_origin_rpy = quat_to_rpy(total_q)
                ...
                print "    Old Line Ending: %s" % oldlineending
                newline = ("\t<origin rpy=\"%.10f %.10f %.10f\" xyz=\"%.10f %.10f %.10f\"/>"% (next_origin_rpy + tuple(origin_xyz)) ) + oldlineending
                print "    transform old line:", joint_lines[origin_ind].strip()
                print "    transform new line:", newline.strip()
                joint_lines[origin_ind] = newline
